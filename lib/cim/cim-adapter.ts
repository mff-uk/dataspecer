...
export interface CimAdapter {
   ...
   * predictable and revertible so the user is able to get CIM iri from
   * PIM iri.
   */
   ...
   * @return Interpreted PIM class or null if the requested CIM class does
   *  not exists
   */
  getClass(cimIri: string): Promise<PimClass | null>;

  /**
   * Get attributes and associations (in distance 1) to a PIM class from CIM
   * specified by CIM iri.
   * @param cimIri iri of CIM entity representing PIM class for which we
   *  look for attributes and associations
   * @return Store containing the PSM class and its surroundings
   */
  getSurroundings(cimIri: string): Promise<CoreResourceReader>;

  /**
   * Returns full (for every class there is also every parent of the class)
   * 'Isa' hierarchy for CIM resource.
   * @param cimIri
