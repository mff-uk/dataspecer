import {RdfSourceWrap} from "../../rdf-source-wrap"
import {RdfResourceAdapter} from "../../rdf-adapter-api";
import {CoreResource, PsmClass} from "../../../../model";
import {loadPsmResource} from "./psm-resource-adapter";
import * as PSM from "./psm-vocabulary";

export class PsmClassAdapter implements RdfResourceAdapter {

  async loadResource(
    source: RdfSourceWrap, resource: CoreResource
  ): Promise<string[]> {
    const types = await source.types();
    if (!types.includes(PSM.CLASS)) {
      return [];
    }
    //
    const psmClass: PsmClass = PsmClass.as(resource);
    const loadFromPim = await loadPsmResource(source, psmClass);
    //
    psmClass.psmExtends = await source.nodesExtended(PSM.HAS_EXTENDS)
    psmClass.psmParts = await source.nodesExtended(PSM.HAS_PART);
    return [...loadFromPim, ...psmClass.psmExtends, ...psmClass.psmExtends];
  }

}
