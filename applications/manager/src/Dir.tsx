...
const Row = ({ iri, parentIri }: { iri: string, parentIri?: string }) => {
  ...
  return <li className="first:border-y last:border-none border-b">
    <div className="flex items-center space-x-4 hover:bg-accent">
      ...
      {resource.types.includes(V1.PSM) && <Button asChild variant={"ghost"} onClick={stopPropagation()}><a href={import.meta.env.VITE_DATA_SPECIFICATION_EDITOR + "/editor?data-specification=" + encodeURIComponent(parentIri ?? "") + "&data-psm-schema=" + encodeURIComponent(iri)}>{t("open")}</a></Button>}
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent>
          ...
          <DropdownMenuItem asChild><a href={import.meta.env.VITE_BACKEND + "/resources/packages/export.zip?iri=" + encodeURIComponent(iri)}><CloudDownload className="mr-2 h-4 w-4" /> {t("export")}</a></DropdownMenuItem>
