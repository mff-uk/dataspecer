...
const PreviewDialog = dialog<{ generatorId: string }>({ fullWidth: true, maxWidth: "xl" }, ({ generatorId, close }) => {
    ...
    return (
        <>
            <DialogContent>
                <SingleArtifactPreview generatorIdentifier={generatorId} />
            </DialogContent>
            <DialogActions>
...
const useSaveOrCopy = (generator: string, save: boolean) => {
    ...
    return useCallback(async () => {
        ...
        try {
            ...
            enqueueSnackbar(
                <>
                    <strong>{t("error")}</strong>: {(error as Error).message}
                </>,
                { variant: "error" }
            );
        }
        ...
        configuration.dataPsmSchemaIri,
        configuration.store,
...
const ArtifactItem: React.FC<{
    ...
    const { t } = useTranslation("artifacts");
    return (
        <MenuItem onClick={() => props.setChecked(!props.checked)}>
    ...
    return (
        <>
            <Button
                ...
                onClick={open}
                ref={ref}
                disabled={!root || root.dataPsmParts.length === 0}
            >
                <PrintTwoToneIcon style={{ marginRight: ".25em" }} />
                {t("button generate load artifacts")}
                <ExpandMoreIcon />
            </Button>
            <StyledMenu id={id} anchorEl={ref.current} keepMounted open={isOpen} onClose={close}>
                <CardContent>
                    <Typography variant={"h5"} component={"div"}>
                        {t("title generate artifacts")}
                    </Typography>
                </CardContent>
                ...
                <MenuNote>
                    <p style={{ maxWidth: "10cm" }}>{t("note use checkbox to live preview")}</p>
                    <p style={{ maxWidth: "10cm" }}>{t("note use manager for all artifacts")}</p>
                </MenuNote>
            </StyledMenu>
