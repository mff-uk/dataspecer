 ...
 */
const defaultRelationshipProfileToIri: RelationshipIriGeneratorStrategy =
  (state) => {
    if (!state.isIriAutogenerated) {
      return state;
    }
    const domainName = NamingConventions.upperCamelCase(
      state.domain.label[state.language] ?? "");
    const name = NamingConventions.lowerCamelCase(
...
const DEFAULT_CONFIGURATION: Configuration = Object.freeze({
  ...
  relationshipProfileToIri: defaultRelationshipProfileToIri,
  hideIdentifier: true,
