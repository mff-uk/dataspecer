...
export function setAttributePositionAction(
  ...
  const oldPosition = node.content.findIndex(attribute => attribute === attributeIdentifer);
  if(oldPosition === -1) {
  ...
  newContent.splice(position, 0, attributeIdentifer);
