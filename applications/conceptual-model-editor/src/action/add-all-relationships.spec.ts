...
import { createDefaultVisualModelFactory, VisualDiagramNode, VisualRelationship } from "@dataspecer/core-v2/visual-model";
...
test("Try to call the action when all edges are missing, then call again, then remove one edge and then call again", () => {
  const {
    visualModel,
    graph,
    modelsAsArray,
