...
import { computeRelatedAssociationsBarycenterAction, convertToEntitiesToDeleteType, EntityToDelete, findTopLevelGroupInVisualModel, getGroupMappings, getRemovedAndAdded, getSelections } from "./utilities";
...
test("Test computeRelatedAssociationsBarycenterAction with enough neighbors", async () => {
  const { visualModel, graph, models, firstModel } = ActionsTestSuite.prepareModelsWithSemanticData(
    0, ActionsTestExportedTypesAndEnums.TestedSemanticConnectionType.Association);
  ...
  ActionsTestSuite.createNewVisualNodeForTesting(visualModel, firstModel.getId(), semanticClasses[0].id, { x: 10, y: 10 });
  ActionsTestSuite.createNewVisualNodeForTesting(visualModel, firstModel.getId(), semanticClasses[1].id, { x: 20, y: 20 });
