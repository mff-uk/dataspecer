...
export function useWdGetEndpoints(
    wdClassId: WdEntityId,
    wdPropertyId: WdEntityId,
    domainsOrRanges: WdDomainsOrRanges,
    ownOrInherited: WdBaseOrInheritOrder,
): UseWdGetEndpointsReturnValue {
    const adapterContext = useContext(WikidataAdapterContext);
    const { data, isError, isLoading } = useQuery(
        [
            "endpoints",
            wdClassId.toString(),
            wdPropertyId.toString(),
            domainsOrRanges,
            ownOrInherited,
        ],
        async () => {
    ...
    const endpoints =
        !isLoading && !queryFailed ? (data as WdClassPropertyEndpoints).classes : undefined;
