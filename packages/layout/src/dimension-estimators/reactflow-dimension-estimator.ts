...
import { ReactflowNodeDimensionsConstantEstimator } from "./constant-dimension-estimator";


/**
 * Sophisticated dimension query handler, which estimates dimensions based node's content.
 */
export class ReactflowNodeDimensionsEstimator implements NodeDimensionQueryHandler {
    getWidth(estimatedNode: INodeClassic): number {
        ...
        const APPROXIMATION_OF_WIDTH_OF_ONE_CHARACTER = ReactflowNodeDimensionsConstantEstimator.getDefaultWidth() / TEST_STRING.length;
    ...
    getHeight(estimatedNode: INodeClassic): number {
        ...
        const BASE_HEIGHT = ReactflowNodeDimensionsConstantEstimator.getDefaultHeight();
